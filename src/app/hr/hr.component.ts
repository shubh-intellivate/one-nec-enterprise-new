...
export class HrComponent implements OnInit  {
  ...
  ngOnInit(){
    this.getIncidentRequest('NEC-HR', '', '', '');
    this.getIncidentRequest('NEC-HR', '', '', 'Open');
    this.getIncidentRequest('NEC-HR', '', '', 'Closed');
    ...
    this.getServiceRequest('NEC-HR', '', '', '');
    this.getServiceRequest('NEC-HR', '', '', 'Open');
    this.getServiceRequest('NEC-HR', '', '', 'Closed');
  ...
  totalAttendedTrainings: any;
  covidDose: any;
  interestedToGetVaccinated: any;
  isVaccinated: any;
  ...
  getLmsTrainings(){
    this.dataService.getLmsTrainings(this.userDetails.UserName, this.userDetails.role).subscribe(
      res => {
        this.totalAttendedTrainings  = res[0].Total_Attended_Training;
      }
    );
  }

  getCovidDetails(){
    this.dataService.getCovidDetails(this.userDetails.UserName, this.userDetails.role).subscribe(
      res => {
        this.covidDose  = res[0].Dose;
        this.interestedToGetVaccinated  = res[0].Interested_To_Get_Vaccinated;
        this.isVaccinated  = res[0].Is_Vaccinated;
